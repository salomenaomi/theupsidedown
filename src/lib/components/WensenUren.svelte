...
<section class="samenvoegen">
  <h2>{wensenUrenInhoud.vraag}</h2>
  <div>
    {#each wensenUrenInhoud.antwoorden as antwoord, index}
    ...
    <button class="terugbutton" on:click={vorigePagina}>
      {wensenUrenInhoud.button1}
    </button>
    {#if antwoorden.length < 1}
      <button disabled class="disabledButton">
        {wensenUrenInhoud.button2}
      </button>
    {:else}
      <button on:click={veranderPagina}> {wensenUrenInhoud.button2} </button>
    {/if}
  </div>
</section>
