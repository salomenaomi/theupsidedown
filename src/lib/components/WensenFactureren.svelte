<script>
  ...
  export let wensenFacturerenInhoud;
...
<section class="samenvoegen">
  <h2>{wensenFacturerenInhoud.vraag}</h2>
  <div>
    {#each wensenFacturerenInhoud.antwoorden as antwoord, index}
    ...
    <button class="terugbutton" on:click={vorigePagina}>
      {wensenFacturerenInhoud.button1}
    </button>
    {#if antwoorden.length < 1}
      <button disabled class="disabledButton">
        {wensenFacturerenInhoud.button2}
      </button>
    {:else}
