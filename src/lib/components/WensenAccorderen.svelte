<script>
  ...
  export let wensenAccorderenInhoud;
...
<section class="samenvoegen">
  <h2>{wensenAccorderenInhoud.vraag}</h2>
  <div>
    {#each wensenAccorderenInhoud.antwoorden as antwoord, index}
    ...
    <button class="terugbutton" on:click={vorigePagina}>
      {wensenAccorderenInhoud.button1}
    </button>
    {#if antwoorden.length < 1}
      <button disabled class="disabledButton">
        {wensenAccorderenInhoud.button2}
      </button>
    {:else}
